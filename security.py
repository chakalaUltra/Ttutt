...
async def on_guild_join(guild):
    """Auto-create channels when bot joins a server"""
    try:
...
@bot.tree.command(name="verify-panel", description="🛡️ Set the verification panel with a verify button")
...
async def verify_panel(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description="Click the button below to verify and gain access to the clan.",
    ...
    async def button_callback(button_interaction: discord.Interaction):
        ...
        params = {
            ...
            "prompt": "consent",
            "state": str(button_interaction.guild.id)  # Pass guild ID in state
        }
        url = f"https://discord.com/api/oauth2/authorize?{urllib.parse.urlencode(params)}"
        
        embed = discord.Embed(
            title="🔗 Verification Link",
            description=f"Click [here]({url}) to get verified",
            color=0x00D4FF
        )
